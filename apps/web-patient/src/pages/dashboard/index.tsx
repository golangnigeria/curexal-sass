...
import { useMyCareJourney, useMyCareRequests } from "@curexal/patient";
...
export function PatientPortalDashboardPage() {
  ...
  const defaultStages = [
    { code: "INTAKE", label: "Intake & Check-In", icon: FileCheck, description: "Demographics & symptoms recorded" },
    { code: "TRIAGE", label: "Triage & Vitals", icon: Activity, description: "Vitals recording & acuity scoring" },
    { code: "CONSULTATION", label: "Doctor Consultation", icon: Stethoscope, description: "Physical or telehealth session" },
    { code: "PRESCRIPTION", label: "E-Prescriptions", icon: Pill, description: "Medications prescribed by physician" },
    { code: "BILLING", label: "Invoicing & Receipts", icon: CreditCard, description: "POS receipt & fee breakdown" },
    { code: "FOLLOW_UP", label: "Care Summary", icon: CheckCircle2, description: "SOAP clinical summary & discharge" },
  ];
  ...
  const getStageStatus = (stageCode: string) => {
    ...
    const found = milestones.find((m: any) => m.stageCode === stageCode);
      ...
      <div className="relative overflow-hidden p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-teal-900/40 via-slate-900 to-slate-950 border border-teal-500/30 shadow-2xl text-white">
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <Link
            ...
            className="flex items-center justify-center gap-2 px-6 py-3 rounded-2xl bg-[#0F766E] hover:bg-[#115E59] text-white text-xs font-bold shadow-lg shadow-teal-900/30 transition-all hover:scale-[1.02] active:scale-[0.98] self-start md:self-center cursor-pointer"
          >
        ...
        <div className="lg:col-span-2 p-6 rounded-3xl bg-white dark:bg-slate-900/80 border border-slate-200/80 dark:border-slate-800 shadow-xl space-y-4">
          ...
          {isRequestsLoading ? (
            ...
            <div className="p-8 text-center space-y-3">
              ...
              <Link
                to="/consultations/new"
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-teal-500/10 text-teal-700 dark:text-teal-300 border border-teal-500/20 text-xs font-bold hover:bg-teal-500/20 transition-all cursor-pointer"
              >
                ...
                >
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-center">
                    {req.preferredMode === "VIDEO" && (
                      <button
                        onClick={() => navigate(`/consultations/${req.id}/room`)}
                        className="flex items-center gap-1.5 px-4 py-2 rounded-xl bg-[#0F766E] hover:bg-[#115E59] text-white text-xs font-bold shadow-md shadow-teal-900/20 transition-all cursor-pointer"
                      >
