...
export function PatientPortalTelehealthRoomPage() {
    ...
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-xs font-mono">
          ...
          <span>Connected (Live Session)</span>
        </div>
      </div>
      ...
      <div className="relative aspect-video rounded-3xl bg-slate-900 border border-slate-800 overflow-hidden shadow-2xl flex flex-col justify-between p-6">
        <div className="absolute inset-0 bg-gradient-to-br from-slate-950 via-slate-900 to-teal-950 flex items-center justify-center">
          <div className="text-center space-y-3">
            <div className="w-20 h-20 rounded-3xl bg-teal-500/20 border border-teal-500/30 text-teal-300 flex items-center justify-center mx-auto shadow-2xl animate-pulse">
              <Stethoscope className="w-10 h-10" />
            </div>
            <div>
              <div className="text-base font-bold text-white">Dr. Attending Physician</div>
              <div className="text-xs text-teal-400 font-medium">General & Internal Medicine Specialist</div>
            </div>
        ...
        <div className="relative z-10 flex items-center justify-center gap-4 py-2">
          ...
          <button
            ...
            className="p-3.5 rounded-2xl bg-rose-500 hover:bg-rose-600 text-white transition-all shadow-lg shadow-rose-500/25 cursor-pointer"
      ...
      {/* Security notice */}
      <div className="p-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-400 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <ShieldCheck className="w-4 h-4 text-emerald-500" />
          <span>This virtual encounter is encrypted and compliant with medical privacy standards.</span>
        </div>
        <span className="text-[11px] text-slate-400">ID: {requestId || "REQ-2026-00412"}</span>
      </div>
    </div>
  );
}
