...
func (m *Module) RegisterRoutes(apiGroup *echo.Group, orgGroup *echo.Group, wspGroup *echo.Group) {
	...
	if m.AuthHandler != nil {
		...
		authGroup.POST("/login", m.AuthHandler.SignIn)
		authGroup.POST("/exchange", m.AuthHandler.ExchangeWorkspaceToken)
		authGroup.POST("/workspaces/select", m.AuthHandler.SelectWorkspaceBranch)
		authGroup.POST("/sign-up", m.AuthHandler.SignUp)
		authGroup.POST("/select-branch", m.AuthHandler.SelectBranch)
		authGroup.POST("/switch-branch", m.AuthHandler.SwitchBranch)
		authGroup.POST("/refresh", m.AuthHandler.RefreshToken)
		authGroup.POST("/sign-out", m.AuthHandler.SignOut)
		authGroup.POST("/logout", m.AuthHandler.SignOut)
